    ...
    return this.authService.token.pipe(
      ...
      switchMap(token => {
        return this.http.get<{ [key: string]: PlaceData }>(
          `https://ionic-angular-course-e80bb-default-rtdb.firebaseio.com/offered-places.json?auth=${token}`
        );
      }),
      map(resData => {
        const places = [];
    ...
    return this.authService.token.pipe(
      ...
      switchMap(token => {
        return this.http.get<PlaceData>(
          `https://ionic-angular-course-e80bb-default-rtdb.firebaseio.com/offered-places/${id}.json?auth=${token}`
        );
      }),
      map(placeData => {
        return new Place(
